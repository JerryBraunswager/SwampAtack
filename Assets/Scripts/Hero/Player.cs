...
public class Player : MonoBehaviour
{
    ...
    private void Update()
    {
        if (_isStop == false & _animator.GetCurrentAnimatorStateInfo(0).IsName(_currentWeapon.WorkLabel) == false)
        {
            if (Input.GetMouseButtonDown(0))
            {
                _animator.speed = _currentWeapon.AttackDelay;
    ...
    {
        _menu.TimeStopped -= TimeStop;
        _menu.OpenPanel(_deadScreen.gameObject);
        _deadScreen.CountMoney();
    }
